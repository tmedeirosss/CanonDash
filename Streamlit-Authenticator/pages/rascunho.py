...
def dashboard(authenticator):
    if st.session_state["authentication_status"]:
        ...
        if st.session_state["name"] is None:
            st.session_state.estado_barra = "collapsed"
            st.write('Aguarde...')
            st.stop()
        else:
...
set_page_config()
get_base64_image("Canon-Logo.png")
...
login(autenticacao())
dashboard(cria_autenticador(carrega_arquivo_config()))
